...
- 중단: USP 카드 4개 가로배열 (번호 + 이미지 placeholder + 설명텍스트), 마지막 카드에 선택적 강조 뱃지 (예: "ONLY" / "NEW" / "단독" — `cta_badge` 필드 재활용, 브랜드명 하드코딩 금지)
...
- 좌하: 원형 배지 — `curriculum_badge` 필드 (Claude가 페이지 맥락에서 생성, 브랜드명 하드코딩 금지)
- 우하: 서브카피 텍스트 + 강조 키워드
...
마케터가 Figma처럼 광고 내 주요 색상을 직접 조정할 수 있도록 컬러 컨트롤 패널을 추가한다.

### 조정 가능한 색상 요소 (레이아웃 공통)

| 요소 | 변수명 | 설명 |
|---|---|---|
| 배경색 | `bgColor` | 기존 bgColor (이미 있음) |
| CTA 바/버튼 색 | `ctaColor` | 기존 ctaColor (이미 있음) |
| 포인트/강조색 | `accentColor` | 헤드라인 키워드 강조, 뱃지 배경 |
| 헤드라인 텍스트색 | `headlineColor` | headline_line1/2 색상 |
| 서브카피 텍스트색 | `subColor` | hook, 본문 등 |
| CTA 텍스트색 | `ctaTextColor` | CTA 버튼 위 텍스트 |

### 컬러 피커 UI 구조

```
[색상 요소 탭] 배경 | CTA | 강조색 | 헤드라인 | 서브
...
[HEX 코드 직접 입력]  # [______] [적용]

[미리보기 실시간 반영] ← 색 바꿀 때마다 iframe 즉시 업데이트
```

### 동작 방식
- 탭으로 요소 전환 → 선택된 요소에 스와치/HEX 적용
- 스와치 클릭 → 해당 요소 색 즉시 변경 + HEX 입력란 동기화
- HEX 입력 → 6자리 완성 시 자동 적용 (실시간)
- "재생성 없이 색만 바꾸기" — 서버 호출 없이 프론트에서 CSS 변수만 교체

### 기술 구현
- HTML 템플릿 내 색상값을 CSS 변수로 통일: `var(--accent)`, `var(--headline)` 등
- iframe의 `contentDocument`에 CSS 변수 주입으로 실시간 반영
- `applyColorToPreview(element, hex)` 함수 — 서버 재호출 없음
- 색상값은 `adcraft_colors` 로컬스토리지에 저장 → 다음 생성 시 자동 적용

### 프리셋 24색 팔레트
```
다크: #0a0a0a, #1a1a2e, #16213e, #0f3460, #1b1b2f, #2d2d2d
컬러: #1877f2, #e84393, #00b140, #ff6b35, #7b2d8b, #f5a623
파스텔: #a8d8ea, #aa96da, #fcbad3, #ffffd2, #b5ead7, #ffdac1
흰/회: #ffffff, #f5f5f5, #e0e0e0, #bdbdbd, #757575, #000000
```
...
모든 HTML 템플릿과 Claude 프롬프트에서 특정 브랜드명을 하드코딩하지 않는다.

**금지 항목:**
- 코드 내 "패스트캠퍼스", "패캠", "FastCampus" 등 특정 브랜드명 하드코딩
- 특정 회사 로고 이미지 내장
- 특정 플랫폼 전용 UI (예: "패캠 커뮤니티" nav 고정)

**허용/권장:**
- `brand` 필드 → 사용자 입력값 또는 Claude가 페이지에서 추출한 값 사용
- SNS UI형 nav 탭 → `platform_name` 필드로 동적 생성 (예: "Q&A | 지식 | 커뮤니티 | JOBS")
- 뱃지/강조 텍스트 → 모두 Claude가 생성하는 동적 필드

**Claude 프롬프트 원칙:**
- 페이지 크롤링 결과에서 브랜드명/강사명을 추출할 때, 특정 브랜드 예시를 프롬프트에 넣지 않는다.
- 예시가 필요한 경우 `[브랜드명]`, `[강사명]` 플레이스홀더 형태로만 표기한다.
